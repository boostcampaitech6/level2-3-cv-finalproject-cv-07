...
class WinEncoderTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, num_encoder_layers=2,
                 ...
                 **kwargs):
        super().__init__()
        encoder_layer_with_attn = EncoderLayer(d_model, nhead, dim_feedforward,
                                                    dropout, activation)
        encoder_layer_with_tokenmixer = EncoderLayer(d_model, nhead, dim_feedforward,
                                                    dropout, activation, False)
        self.encoder = TransformerEncoder(encoder_layer_with_attn, encoder_layer_with_tokenmixer, num_encoder_layers, **kwargs)
    ...
    def forward(self, src, pos_embed, mask):
        ...
        for idx, enc_win_size in enumerate(self.enc_win_list):
            ...
            # encoder forward
            output = self.encoder.single_forward(memeory_win, origin_shape, enc_win_size, src_key_padding_mask=mask_win, pos=pos_embed_win, layer_idx=idx)
...
class TransformerEncoder(nn.Module):
    ...
    """
    def __init__(self, encoder_layer_attn, encoder_layer_tokenmixer, num_layers, **kwargs):
        super().__init__()
        self.layers = _get_clones(num_layers, module1=encoder_layer_tokenmixer, module2=encoder_layer_attn)
...
class EncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=512, dropout=0.0,
        ...
        if self.attn:
            ...
            self.token_mixer = DepthwiseConv(d_model)
        # feedforward layer
    ...
    def forward(self, src,
        ...
        if self.attn:
            ...
            src_ = src.permute(1, 2, 0).reshape(-1, c, w_h, w_w)
            src2 = self.token_mixer(src_)
...
def _get_clones(N, module1:Optional[nn.Module] = None, module2: Optional[nn.Module] = None):
    """
    module1 is a module with a self attention layer
    module2 is a module with a token mixer layer
    module2's default type is None
    """
    if module2 is None:
        return nn.ModuleList([copy.deepcopy(module1) for i in range(N)])
    else:
        return nn.ModuleList([copy.deepcopy(module1) if i < 2 else copy.deepcopy(module2) for i in range(N)])

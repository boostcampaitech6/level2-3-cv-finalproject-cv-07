...
def enc_win_partition(src, pos_embed, mask, enc_win_h, enc_win_w):
    ...
    pos_embed_win = window_partition(
        pos_embed, window_size_h=enc_win_h, window_size_w=enc_win_w
    )
    mask_win = (
        window_partition(
            mask.unsqueeze(1), window_size_h=enc_win_h, window_size_w=enc_win_w
        )
        .squeeze(-1)
        .permute(1, 0)
    )
...
def enc_win_partition_reverse(windows, window_size_h, window_size_w, H, W):
    ...
    Origin window: window_size*window_size, num_windows*B, C
    """

    B = int(windows.shape[1] / (H * W / window_size_h / window_size_w))
    x = windows.permute(1, 0, 2).reshape(
...
def enc_win_partition_reverse_pooling(windows, window_size_h, window_size_w, H, W):
    """
    pooling window: B * (H * W / (w_h / w_w)), C, w_h, w_w
    """
...
def window_partition_pooling(x, window_size_h, window_size_w):
    """
    window-rize input
    """
    B, C, H, W = x.shape
    ...
    windows = x.permute(0, 2, 4, 1, 3, 5).reshape(
        -1, C, window_size_h, window_size_w
    )  # num_windows*B, C, window_h_size, window_w_size
...
def window_partition(x, window_size_h, window_size_w):
    ...
    x = x.reshape(
        B, H // window_size_h, window_size_h, W // window_size_w, window_size_w, C
    )
    windows = x.permute(0, 1, 3, 2, 4, 5).reshape(
        -1, window_size_h, window_size_w, C
    )  #
    windows = windows.view(-1, window_size_h * window_size_w, C).permute(
        1, 0, 2

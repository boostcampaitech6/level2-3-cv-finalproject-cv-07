...
class BasePETCount(nn.Module):
    ...
    def __init__(
        self, backbone, num_classes, quadtree_layer="sparse", args=None, **kwargs
    ):
        super().__init__()
        ...
        self.pq_stride = (
            args.sparse_stride if quadtree_layer == "sparse" else args.dense_stride
        )
    ...
    def points_queris_embed(self, samples, stride=8, src=None, **kwargs):
        ...
        points_queries = torch.vstack([shift_y.flatten(), shift_x.flatten()]).permute(
            1, 0
        )  # 2xN --> Nx2
        h, w = shift_x.shape
        ...
        shift_y_down, shift_x_down = (
            points_queries[:, 0] // stride,
            points_queries[:, 1] // stride,
        )
    ...
    def points_queris_embed_inference(self, samples, stride=8, src=None, **kwargs):
        ...
        query_embed_win = window_partition(
            query_embed, window_size_h=dec_win_h, window_size_w=dec_win_w
        )
        points_queries_win = window_partition(
            points_queries, window_size_h=dec_win_h, window_size_w=dec_win_w
        )
        ...
        div_win = window_partition(
            div.unsqueeze(1), window_size_h=dec_win_h, window_size_w=dec_win_w
        )
    ...
    def get_point_query(self, samples, features, **kwargs):
        ...
        if "train" in kwargs:
            query_embed, points_queries, query_feats = self.points_queris_embed(
                samples, self.pq_stride, src, **kwargs
            )
            query_embed = query_embed.flatten(2).permute(
                2, 0, 1
            )  # NxCxHxW --> (HW)xNxC
            v_idx = None
        else:
            query_embed, points_queries, query_feats, v_idx = (
                self.points_queris_embed_inference(
                    samples, self.pq_stride, src, **kwargs
                )
            )
...
class PET(nn.Module):
    ...
    def __init__(self, backbone, num_classes, args=None):
        ...
        context_w, context_h = context_patch[0] // int(
            self.encode_feats[:-1]
        ), context_patch[1] // int(self.encode_feats[:-1])
    ...
    def compute_loss(self, outputs, criterion, targets, epoch, samples):
        ...
        # compute loss
        if epoch >= warmup_ep:
            loss_dict_sparse = criterion(
                output_sparse, targets, div=outputs["split_map_sparse"]
            )
            loss_dict_dense = criterion(
                output_dense, targets, div=outputs["split_map_dense"]
            )
        else:
        ...
        loss_pq_sparse = sum(
            loss_dict_sparse[k] * weight_dict_sparse[k]
            for k in loss_dict_sparse.keys()
            if k in weight_dict_sparse
        )
        ...
        loss_pq_dense = sum(
            loss_dict_dense[k] * weight_dict_dense[k]
            for k in loss_dict_dense.keys()
            if k in weight_dict_dense
        )
    ...
    def forward(self, samples: NestedTensor, **kwargs):
        ...
        # feature projection
        features["4x"] = NestedTensor(
            self.input_proj[0](features["4x"].tensors), features["4x"].mask
        )
        features["8x"] = NestedTensor(
            self.input_proj[1](features["8x"].tensors), features["8x"].mask
        )
    ...
    def pet_forward(self, samples, features, pos, **kwargs):
        ...
        if "train" in kwargs or (split_map_sparse > 0.5).sum() > 0:
            ...
            outputs_sparse = self.quadtree_sparse(
                samples, features, context_info, **kwargs
            )
        else:
            outputs_sparse = None
        ...
        if "train" in kwargs or (split_map_dense > 0.5).sum() > 0:
            ...
            outputs_dense = self.quadtree_dense(
                samples, features, context_info, **kwargs
            )
        else:
            outputs_dense = None
    ...
    def test_forward(self, samples, features, pos, **kwargs):
        ...
        if outputs["sparse"] is not None:
            out_sparse_scores = torch.nn.functional.softmax(
        ...
        if outputs["dense"] is not None:
            out_dense_scores = torch.nn.functional.softmax(
        ...
        for name in list(output_names):
            if "pred" in name:
                ...
                div_out[name] = (
                    out_sparse[name] if out_sparse is not None else out_dense[name]
                )
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_points, log=True, **kwargs):
        ...
        if "div" in kwargs:
            ...
            loss_ce_nondiv = (raw_ce_loss * weights * non_div_mask).sum() / (
                (weights * non_div_mask).sum() + eps
            )
    ...
    def loss_points(self, outputs, targets, indices, num_points, **kwargs):
        ...
        if "div" in kwargs:
            ...
            pt_ds_idx = torch.cat(
                [torch.where(idx[0] == bs_id)[0] for bs_id in img_ds_idx]
            )
            pt_sp_idx = torch.cat(
                [torch.where(idx[0] == bs_id)[0] for bs_id in img_sp_idx]
            )
            ...
            loss_points_div_sp = loss_points_div[pt_sp_idx].sum() / (
                len(pt_sp_idx) + eps
            )
            loss_points_div_ds = loss_points_div[pt_ds_idx].sum() / (
                len(pt_ds_idx) + eps
            )
            ...
            non_div_mask = split_map <= div_thrs
            loss_points_nondiv = (
                loss_points_raw * non_div_mask[idx].unsqueeze(-1)
            ).sum() / (non_div_mask[idx].sum() + eps)

            # final point loss
            losses["loss_points"] = (
                loss_points_div_sp + loss_points_div_ds + loss_points_nondiv
            )
        else:
    ...
    def forward(self, outputs, targets, **kwargs):
        ...
        num_points = torch.as_tensor(
            [num_points], dtype=torch.float, device=next(iter(outputs.values())).device
        )
        ...
        for loss in self.losses:
            losses.update(
                self.get_loss(loss, outputs, targets, indices, num_points, **kwargs)
            )
        return losses

...
from util.misc2 import NestedTensor
...
def visualization(samples, targets, pred, vis_dir, split_map=None):
    ...
    for idx in range(images.shape[0]):
        ...
        if split_map is not None:
            ...
            split_map = cv2.resize(
                split_map, (imgW, imgH), interpolation=cv2.INTER_NEAREST
            )
...
def train_one_epoch(
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        outputs = model(
            samples, epoch=epoch, train=True, criterion=criterion, targets=targets
        )
        loss_dict, weight_dict, losses = (
            outputs["loss_dict"],
            outputs["weight_dict"],
            outputs["losses"],
        )
...
def evaluate(model, data_loader, device, epoch=0, vis_dir=None):
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        wanted = predict_cnt - gt_cnt
        print(
            f"prd_cnt: {predict_cnt:4d}, GT_cnt: {gt_cnt:4d}, value: {wanted:4d} ",
            end="",
        )

        # compute error
        mae = abs(predict_cnt - gt_cnt)
        # mae = predict_cnt - gt_cnt

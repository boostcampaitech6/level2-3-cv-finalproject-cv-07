    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.return_intermediate = (
            kwargs["return_intermediate"] if "return_intermediate" in kwargs else False
        )
    ...
    def forward(self, src, pos_embed, mask):
        ...
        for idx, enc_win_size in enumerate(self.enc_win_list):
            ...
            memeory_win, pos_embed_win, mask_win = enc_win_partition(
                memeory, pos_embed, mask, enc_win_h, enc_win_w
            )
...
class WinDecoderTransformer(nn.Module):
    ...
    def decoder_forward(
        ...
        **kwargs,
        ...
        query_embed_win = window_partition(
            query_embed_, window_size_h=dec_win_h, window_size_w=dec_win_w
        )
        tgt = window_partition(
            query_feats, window_size_h=dec_win_h, window_size_w=dec_win_w
        )
        ...
        hs_tmp = [
            window_partition_reverse(hs_w, dec_win_h, dec_win_w, qH, qW)
            for hs_w in hs_win
        ]
        hs = torch.vstack([hs_t.unsqueeze(0) for hs_t in hs_tmp])
        return hs

    def decoder_forward_dynamic(
        self,
        query_feats,
        query_embed,
        memory_win,
    ...
    def forward(self, src, pos_embed, mask, pqs, **kwargs):
        ...
        if "test" in kwargs:
            ...
            hs = self.decoder_forward_dynamic(
                query_feats,
                query_embed,
                memory_win,
                pos_embed_win,
...
def CopyParams(base_layer, target_layer, decoder=False):
    if decoder:
        if target_layer.self_attn._qkv_same_embed_dim:
        ...
        if target_layer.multihead_attn._qkv_same_embed_dim:
            target_layer.multihead_attn.in_proj_weight = (
                base_layer.multihead_attn.in_proj_weight
            )
        else:
            target_layer.multihead_attn.k_proj_weight = (
                base_layer.multihead_attn.k_proj_weight
            )
            target_layer.multihead_attn.v_proj_weight = (
                base_layer.multihead_attn.v_proj_weight
            )
            target_layer.multihead_attn.q_proj_weight = (
                base_layer.multihead_attn.q_proj_weight
            )

        target_layer.multihead_attn.out_proj = base_layer.multihead_attn.out_proj

        if base_layer.multihead_attn.bias_k is not None:
            target_layer.multihead_attn.bias_k = base_layer.multihead_attn.bias_k
        if base_layer.multihead_attn.bias_v is not None:
            target_layer.multihead_attn.bias_v = base_layer.multihead_attn.bias_v
        if target_layer.multihead_attn.in_proj_bias is not None:
            target_layer.multihead_attn.in_proj_bias = (
                base_layer.multihead_attn.in_proj_bias
            )

        target_layer.self_attn.out_proj = base_layer.self_attn.out_proj
        target_layer.multihead_attn.out_proj = base_layer.multihead_attn.out_proj
        target_layer.linear1 = base_layer.linear1
        target_layer.linear2 = base_layer.linear2
        target_layer.norm1 = base_layer.norm1
        target_layer.norm2 = base_layer.norm2
        target_layer.norm3 = base_layer.norm3
        target_layer.activation = base_layer.activation
        target_layer.nhead = base_layer.nhead
        target_layer.d_model = base_layer.d_model
...
def get_sequence_layers(base_layer, num_layers, decoder=False):
    layers = []
    m = num_layers // 2
    for i in range(1, num_layers + 1):
        if (i - 1) % (num_layers // m) == 0:
            layers.append(copy.deepcopy(base_layer))
        else:
            base_n = i - 2
            encoder_layer = copy.deepcopy(layers[base_n])
            encoder_layer = CopyParams(layers[base_n], encoder_layer, decoder)
            layers.append(encoder_layer)
    return nn.ModuleList(layers)


def get_cycle_layers(base_layer, num_layers):
    layers = []
    m = num_layers // 2
    for i in range(1, num_layers + 1):
        if i <= m:
            ...
            base_n = (i - 1) % 2
            encoder_layer = copy.deepcopy(layers[base_n])
            encoder_layer = CopyParams(layers[base_n], encoder_layer)
            layers.append(encoder_layer)
    return nn.ModuleList(layers)


def get_cycle_rev_layers(base_layer, num_layers):
    layers = []
    m = num_layers // 2
    for i in range(1, num_layers + 1):
        if i <= m:
            layers.append(copy.deepcopy(base_layer))
        elif i <= (m * ((num_layers // m) - 1)):
            base_n = (i - 1) % 2
            encoder_layer = copy.deepcopy(layers[base_n])
            encoder_layer = CopyParams(layers[base_n], encoder_layer)
            layers.append(encoder_layer)
...
class TransformerEncoder(nn.Module):
    """
    Base Transformer Encoder with cycle_rev mode
    """

    def __init__(self, encoder_layer, num_layers, **kwargs):
        super().__init__()
        self.layers = get_cycle_rev_layers(encoder_layer, num_layers)
        """
        Transformer Encoder with other modes

        self.layers = _get_clones(encoder_layer, num_layers)
        self.layers = get_sequence_layers(encoder_layer, num_layers)
        self.layers = get_cycle_layers(encoder_laeyr, numlayers)
        """
...
class TransformerDecoder(nn.Module):
    """
    Base Transformer Decoder with normal mode
    """

    def __init__(self, decoder_layer, num_layers, norm=None, return_intermediate=False):
        super().__init__()
        """
        Transformer Decoder with sequence mode

        self.layers = get_sequence_layers(decoder_layer, num_layers, decoder=True)
        """
...
class EncoderLayer(nn.Module):
    def __init__(
        self, d_model, nhead, dim_feedforward=512, dropout=0.0, activation="relu"
    ):
        super().__init__()

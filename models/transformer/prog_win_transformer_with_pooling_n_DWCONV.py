...
class WinEncoderTransformer(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        encoder_layer_with_dwconv = EncoderLayer(
            d_model, nhead, dim_feedforward, dropout, activation
        )
        encoder_layer_with_pool = EncoderLayer(
            d_model, nhead, dim_feedforward, dropout, activation, True
        )
        self.encoder = TransformerEncoder(
            encoder_layer_with_dwconv,
            encoder_layer_with_pool,
            num_encoder_layers,
            **kwargs,
        )
    ...
    def forward(self, src, pos_embed, mask):
        ...
        for idx, enc_win_size in enumerate(self.enc_win_list):
            ...
            memeory_win = window_partition_pooling(memeory, enc_win_h, enc_win_w)

            # encoder forward
            output = self.encoder.single_forward(
                memeory_win, origin_shape, enc_win_size, layer_idx=idx
            )
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, encoder_layer_dwconv, encoder_layer_pool, num_layers, **kwargs):
        super().__init__()
        self.layers = _get_clones(
            num_layers, module1=encoder_layer_dwconv, module2=encoder_layer_pool
        )
...
class EncoderLayer(nn.Module):
    def __init__(
        ...
        if self.pooling:
            ...
            self.token_mixer = DepthwiseConv(d_model, kernel_size=3)
            # self.bnorm = nn.BatchNorm2d(d_model)
...
def _get_clones(
    ...
    """
    module1 is a module with a DW Conv layer
    module2 is a module with a Pooling layer
    ...
    if module2 is None:
        ...
        """
        Default
        Encoder layers: DW Conv -> pooling
        """
        return nn.ModuleList(
            [
                copy.deepcopy(module1) if i < 1 else copy.deepcopy(module2)
                for i in range(N)
            ]
        )
